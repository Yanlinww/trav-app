      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-10">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-xl font-bold text-[#30485f] sm:text-2xl">{heading}</h2>
          </div>
          <div className="flex flex-wrap items-center justify-start gap-2 sm:justify-end"><button type="button" onClick={() => switchViewMode('discover')} className={`inline-flex items-center gap-1.5 rounded-xl px-2.5 py-2 text-[11px] font-bold transition sm:px-3 sm:text-xs ${viewMode === 'discover' ? 'bg-[#5e7891] text-white shadow-sm' : 'bg-white text-[#698499] hover:bg-[#eaf2f7]'}`}><Globe2 size={14} />探索行程</button><button type="button" onClick={() => switchViewMode('saved')} className={`inline-flex items-center gap-1.5 rounded-xl px-2.5 py-2 text-[11px] font-bold transition sm:px-3 sm:text-xs ${viewMode === 'saved' ? 'bg-[#5e7891] text-white shadow-sm' : 'bg-white text-[#698499] hover:bg-[#eaf2f7]'}`}><Bookmark size={14} className={viewMode === 'saved' ? 'fill-current' : ''} />我的收藏</button><label className="inline-flex items-center rounded-xl border border-[#d7e4ec] bg-white px-2 text-[11px] font-bold text-[#698499] sm:text-xs"><span className="sr-only">行程排序</span><select value={sortBy} onChange={(event) => updateSort(event.target.value as ItinerarySort)} className="cursor-pointer bg-transparent py-2 text-[11px] font-bold outline-none sm:text-xs">{itinerarySortOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>{!isLoading && <span className="inline-flex items-center gap-1 text-xs text-[#91a6b7]">{isRefreshing && <Loader2 className="size-4 animate-spin" />}共 {itineraries.length} 份</span>}</div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid items-start grid-cols-2 gap-3 sm:items-stretch sm:gap-5 lg:grid-cols-3 xl:grid-cols-4">
            {itineraries.map((itinerary) => (
              <article key={itinerary.id} role="button" tabIndex={0} aria-label={`預覽行程：${itinerary.title}`} onClick={(event) => { if ((event.target as HTMLElement).closest('button, a')) return; void openPreview(itinerary.id); }} onKeyDown={(event) => { if (event.target !== event.currentTarget || (event.key !== 'Enter' && event.key !== ' ')) return; event.preventDefault(); void openPreview(itinerary.id); }} className="group flex self-start cursor-pointer flex-col overflow-hidden rounded-2xl border border-[#dce7ef] bg-white shadow-[0_6px_18px_rgba(66,96,120,0.06)] transition duration-300 hover:-translate-y-1 hover:border-[#c4d8e6] hover:shadow-[0_14px_28px_rgba(66,96,120,0.12)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6f91aa] focus-visible:ring-offset-2 sm:h-full">
