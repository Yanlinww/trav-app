        ...
        {isLoading ? (
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {itineraries.map((itinerary) => (
              <article key={itinerary.id} role="button" tabIndex={0} aria-label={`預覽行程：${itinerary.title}`} onClick={(event) => { if ((event.target as HTMLElement).closest('button, a')) return; void openPreview(itinerary.id); }} onKeyDown={(event) => { if (event.target !== event.currentTarget || (event.key !== 'Enter' && event.key !== ' ')) return; event.preventDefault(); void openPreview(itinerary.id); }} className="group flex h-full cursor-pointer flex-col overflow-hidden rounded-2xl border border-[#dce7ef] bg-white shadow-[0_6px_18px_rgba(66,96,120,0.06)] transition duration-300 hover:-translate-y-1 hover:border-[#c4d8e6] hover:shadow-[0_14px_28px_rgba(66,96,120,0.12)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6f91aa] focus-visible:ring-offset-2">
